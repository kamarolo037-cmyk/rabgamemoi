...
async function upgradeSlave(slaveId){
  await withBusy(async () => {
    try {
      await callAction("upgrade", { slaveId });
      toast("Раб улучшен!");
      closeModal();
      await collect(true);
      render();
...
async function openSlave(id){
  ...
  showModal(`
    ...
    <button onclick="sell('${esc(s.id)}')" style="margin-bottom:10px;width:100%;">
      Продать за ${fmt(sellPrice)} ₽${locked ? " (замок снимется)" : ""}
    </button>
...
async function openProfile(id){
  ...
  showModal(`
    ...
    ${canBuy ? `<button onclick="buy('${esc(p.id)}')" style="width:100%;margin-top:14px;">Купить за ${fmt(buyPrice(p.earn_rate))} ₽</button>` : ""}
  `);
}
