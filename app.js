...
async function openSlave(id){
  ...
  showModal(`
    ...
    ${locked
      ? `<div class="muted" style="margin-bottom:12px;color:var(--red);"><b>🔒 Замок надет!</b> Не получится продать и надеть новый замок до ${new Date(s.locked_until).toLocaleString("ru-RU")}</div>`
